...
def get_model(config, vocab_src_len, vocab_tgt_len):
    ...
    ### Number of heads and number of layers can be reduced depending upon the GPU capacity and model performance expected. 
    return model
...
def train_model(config):
    ...
    for epoch in range(initial_epoc, config['num_epoch']):
        model.train()
        ...
        for batch in batch_iterator:
            ...
            decoder_mask = batch['decoder_mask'].to(device) #(Batch,1,seqlen, seqlen)

            ## run the tensor throuhg the transformer
            encoder_output = model.encode(encoder_input, encoder_mask) # batch, seqlen, d_model
            ...
            loss = loss_fn(proj_output.view(-1, tokenizer_tgt.get_vocab_size()), label.view(-1)) ## we want to compare the output tnesors with the original 
            ...
            #optimize the gradiant nad update the wrights
            optimizer.step()
            optimizer.zero_grad()
        ...
        # save the optimizer state also to make the training resumable
        torch.save({
            'epoch': epoch,
            'mode;_state_dict': model.state_dict(),
